...
graph TB
    subgraph "Frontend (React + Vite)"
        A[Voice Assistant UI] --> B[Orb Visualization]
        A --> C[Mic Button]
        A --> D[Upload Button]
        A --> E[Subtitle Display]
    ...
    subgraph "Backend (FastAPI)"
        J[Main API Server] --> K[Upload Router]
        J --> L[QA Router]
        J --> M[Flow Router]
        J --> N[Interview Router]
        J --> O[Conversation Router]
        
        K --> P[PDF Reader Service]
        L --> Q[RAG Pipeline]
        M --> R[Orchestrator]
        N --> S[Interview Service]
        O --> T[Conversational AI]
        
        P --> U[Text Extraction]
        Q --> V[Vector Store]
        R --> W[LLM Service]
        S --> X[Question Generator]
        T --> Y[Context Manager]
    end
    
    subgraph "AI Services"
        Z[Groq Whisper STT] --> AA[Speech Recognition]
        BB[Google Gemini] --> CC[Text Generation]
        DD[Google TTS] --> EE[Speech Synthesis]
        FF[ChromaDB] --> GG[Vector Storage]
        HH[Sentence Transformers] --> II[Embeddings]
    end
    
    subgraph "Storage"
        JJ[(MySQL Database)] --> KK[PDF Metadata]
        LL[Cloudinary] --> MM[File Storage]
        FF --> NN[Document Vectors]
    end
